...
export default function AdminProfile() {
  ...
  const onPasswordSubmit = async (data: PasswordFormData) => {
    ...
    try {
      ...
      Alert.alert(t('common.error'), t(errorMessage) || t('admin.profile.passwordError') || t('agent.profile.passwordError'));
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <ScreenContainer variant="background">
        <View style={[styles.loadingContainer, { paddingTop: insets.top + 100 }]}>
          <ThemedText variant="error" size="base">
            {t('admin.profile.loadError') || t('agent.profile.loadError') || 'Erreur lors du chargement du profil'}
          </ThemedText>
        </View>
      </ScreenContainer>
    );
  ...
  const profileInfo = [
    ...
    ...(profile.department ? [{
      ...
      label: t('admin.profile.department') || t('agent.profile.department') || 'Département',
